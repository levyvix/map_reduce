...
def consolidate_content(fp, R):
    ...
    return reduce(join_breaks, content, {"first": [], "second": [], "third": []})


def make_line(parts):
    return " ".join(
        [choice(parts["first"]), choice(parts["second"]), choice(parts["third"])]
    )
...
if __name__ == "__main__":
    ...
    # max size 10mb
    generate_poems(author_a, author_b, max_size=10000)
